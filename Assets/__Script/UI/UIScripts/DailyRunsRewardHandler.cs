...
public class DailyRunsRewardHandler : MonoBehaviour
{
    ...
    [SerializeField] private ChestShopScriptableObject chestShopScriptableObject;
   

    public Action dailyChallengeRunsUpdate { get; set; }



    private void Update() {
        if (Input.GetKeyDown(KeyCode.Numlock)) {
            AddRunsToThisTask(Random.Range(10, 20));
        }
    }
    ...
    private void ActivateDailyRunsChallengeAgain()
	{
        ...
        dailyChallengeRunsUpdate?.Invoke();
    }

    public void AddRunsToThisTask(int _runs)
	{
	...
	{
        hasClaimedDailyRunsReward = true;
        UIManager.Instance.ui_ChestOpping.ActavetedShopBaseChest(chestShopScriptableObject ,0);
