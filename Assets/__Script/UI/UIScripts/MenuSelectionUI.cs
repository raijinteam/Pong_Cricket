        ...
        for (int i = 0; i < all_MenuOptions.Length; i++)
        {
			...
			if (i == _menuIndex)
			{
                ...
                if (all_SelectedMenuPanels[i].activeSelf) {
                    ...
                    GameManager.Instance.mini_GameSpawner.gameObject.SetActive(false);
                    if (i == 0) {
                    ...
                    if (i == 4) {
                        if (DataManager.Instance.skipIts > 0 || AdsManager.instance.IsRewardAdLoad) {
                            GameManager.Instance.StartMinigame();
                        }
                        else {
                            UIManager.Instance.ui_HomeScreen.gameObject.SetActive(true);
                            UIManager.Instance.spawnPopup("No Enough Skip");
                        }
