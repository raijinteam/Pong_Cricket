...
public class DailyRewardHandler : MonoBehaviour
{
	...
	{
		isDailyRewardActive = false;
        GetDailyReawardOfDay(currentDayIndexForDailyReward);
    ...
    private void GetDailyReawardOfDay(int index) {
		Debug.Log(index);
		switch (all_RewardType[index]) {
			case DailyRewardType.Coins:
				Debug.Log("Coin Incresed..." + all_RewardAmounts[index]);
				DataManager.Instance.IncresedCoin(all_RewardAmounts[index]);
				break;
			case DailyRewardType.Gems:
                Debug.Log("Gems Incresed..." + all_RewardAmounts[index]);
                DataManager.Instance.Incresedgems(all_RewardAmounts[index]);
                break;
			case DailyRewardType.SkipIts:
                Debug.Log("Skip Incresed..." + all_RewardAmounts[index]);
                DataManager.Instance.IncresedSkipIt(all_RewardAmounts[index]);
                break;
			case DailyRewardType.Chest:
                Debug.Log("Chest Incresed..." + all_RewardAmounts[index]);
