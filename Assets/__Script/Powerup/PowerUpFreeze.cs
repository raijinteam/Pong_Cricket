...
public class PowerUpFreeze : Powerup {
    ...
    public override void ActivtedMyPowerup(AbilityType type, bool Isplayer) {
        ...
        if (Isplayer) {
            ...
            GameManager.Instance.CurrentGamePlayerAI.playerAi.ActivateFreezePowerup(flt_FreezTime, flt_IntervalTime);

        }
        else {

            GameManager.Instance.CurrentGamePlayer.player.ActivateFreezePowerup(flt_FreezTime, flt_IntervalTime);
    ...
    public override void DeActivtedMyPowerup() {
        if (hasPlayerActivatedPowerup) {
            ...
            GameManager.Instance.CurrentGamePlayerAI.playerAi.DeActivateFreezePowerup();
        }
        else {

            GameManager.Instance.CurrentGamePlayer.player.DeActivateFreezePowerup();
        }
    }
}
