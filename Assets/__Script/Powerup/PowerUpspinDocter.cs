...
public class PowerUpspinDocter : Powerup {
    ...
    public override void ActivtedMyPowerup(AbilityType type, bool Isplayer) {
        ...
        if (hasPlayereActvetdPowerup) {
            GameManager.Instance.CurrentGamePlayer.playerCollsion.ActivetedSpinDocter(per_SwingForce);
        }
        else {
            GameManager.Instance.CurrentGamePlayerAI.playerCollsion.ActivetedSpinDocter(per_SwingForce);
        }
    ...
    public override void DeActivtedMyPowerup() {
        ...
        if (hasPlayereActvetdPowerup) {
            GameManager.Instance.CurrentGamePlayer.playerCollsion.DeActivetedSpinDocter();
        }
        else {
            GameManager.Instance.CurrentGamePlayerAI.playerCollsion.DeActivetedSpinDocter();
        }
    }
}
