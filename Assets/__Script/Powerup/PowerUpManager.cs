...
public class PowerUpManager : MonoBehaviour {
    ...
    private void SpawnPowerup(bool _IsPlayerCollected, PlayerState _currentState) {
        ...
        for (int i = 0; i < all_Powerup.Length; i++) {
            ...
            else if (!AbilityManager.Instance.IsAbilityUnlockedWithType(all_Powerup[i].myType)) {
                continue;
            }
            else if (all_Powerup[i].PowerupStatus == MyPowerUp.Both) {
                list_Spawner.Add(all_Powerup[i]);
            }
            else if(_currentState == PlayerState.BatsMan && all_Powerup[i].PowerupStatus == MyPowerUp.BatsMan) {
               
                list_Spawner.Add(all_Powerup[i]);

            }
            else if (_currentState == PlayerState.Bowler && all_Powerup[i].PowerupStatus == MyPowerUp.Bowlwer) {
                list_Spawner.Add(all_Powerup[i]);
            }
        }
